...
def main() -> int:
    ...
    data_store = s3.Bucket(name=os.environ["SCW_DATA_STORE"])  # type: ignore
    data_store.download_file(
        os.environ["DATA_FILE_NAME"], "./data/" + os.environ["DATA_FILE_NAME"]
    )
    data = ml.load_data("./data/" + os.environ["DATA_FILE_NAME"])
    ...
    pickle.dump(classifier, open(os.environ["MODEL_FILE_NAME"], "wb"))
    model_registry = s3.Bucket(name=os.environ["SCW_MODEL_REGISTRY"])  # type: ignore
    model_registry.upload_file(
        Filename="/ml-job/" + os.environ["MODEL_FILE_NAME"],
    ...
    performance_monitor = s3.Bucket(name=os.environ["SCW_PERF_MONITOR"])  # type: ignore
    performance_monitor.upload_file(
        Filename="/ml-job/performance_metrics.pkl", Key="performance_metrics.pkl"
    )

    # save roc_auc plot
    ml.save_roc_plot(classifier, X_test, y_test)
    performance_monitor.upload_file(
        Filename="/ml-job/roc_auc_curve.png", Key="roc_auc_curve.png"
    )

    # save confusion matrix
    ml.save_confusion_matrix_plot(classifier, X_test, y_test)
    performance_monitor.upload_file(
        Filename="/ml-job/confusion_matrix.png", Key="confusion_matrix.png"
    )
