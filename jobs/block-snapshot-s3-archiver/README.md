...
This tool automatically finds available snapshots of Scaleway Block Storage volumes, exports them to a specified S3 bucket in `.qcow2` format, and deletes the original snapshot to optimize storage costs. It's designed to run as a Serverless Job on Scaleway and skips snapshots that have already been archived.
...
docker login rg.fr-par.scw.cloud/block-snapshot-s3-archiver -u nologin --password-stdin <<< "$SCW_SECRET_KEY"

# Here we build the image to push
docker buildx build --platform linux/amd64 -t rg.fr-par.scw.cloud/block-snapshot-s3-archiver/block-snapshot-s3-archiver:v1 .

# Push the image online to be used on Serverless Jobs
docker push rg.fr-par.scw.cloud/block-snapshot-s3-archiver/block-snapshot-s3-archiver:v1
```
> [!TIP]
